  ...
  for (size_t I = 0; I < N; I += blockSize) {
    for (size_t J = I; J < N; J += blockSize) {
      for (size_t i = I; i < I + blockSize; i++) {
        // Pairwise force calculation
        for (size_t j = (J > i) ? J : (i+1); j < J + blockSize; j++) {
          // Calculate particle distance
          const double dx = particles[j].x - particles[i].x;
          const double dy = particles[j].y - particles[i].y;
          const double dz = particles[j].z - particles[i].z;
        ...
        // After processing all of the j samples
        if (J == (N - blockSize)){
          // Update position of particle i with Leap-frog scheme
          const double invM = 1. / particles[i].m;
          particles[i].vx += invM * particles[i].fx * dt;
          particles[i].vy += invM * particles[i].fy * dt;
          particles[i].vz += invM * particles[i].fz * dt;
